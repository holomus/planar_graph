...
class HeadHunterRuntimeService extends RuntimeService {
      ...
      if (conn.getResponseCode == HttpURLConnection.HTTP_OK
       ...
       || conn.getResponseCode == HttpURLConnection.HTTP_NO_CONTENT) {
        ...
        if (conn.getErrorStream != null) {
          ...
          ErrorRuntimeResult("Head Hunter response error status code = " + conn.getResponseCode)
        }
      }
    } catch {
      case ex: Exception =>
        ErrorRuntimeResult("connection fail with Head Hunter server " + ex.getMessage)
    } finally {
  ...
  override def run(detail: Map[String, Any], data: String): RuntimeResult = {
    ...
    val headers = if (!isEmpty(accessToken))
                     Map("content-type" -> "application/json", "Authorization" -> ("Bearer " + accessToken))
                  else
