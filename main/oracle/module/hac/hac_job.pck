create or replace package Hac_Job is
  ...
  ----------------------------------------------------------------------------------------------------
  Procedure Person_Sync_Request_Procedure(o_Output out Array_Varchar2);
  ----------------------------------------------------------------------------------------------------
  Procedure Person_Sync_Response_Procedure(i_Input Array_Varchar2);
end Hac_Job;
/
create or replace package body Hac_Job is
  ...
  ----------------------------------------------------------------------------------------------------
  Procedure Person_Sync_Request_Procedure(o_Output out Array_Varchar2) is
    v_Company   Gmap;
    v_Companies Glist := Glist();
    result      Glist := Glist();
  begin
    for r in (select Dt.Company_Id,
                     cast(collect(to_number(to_char(Dt.Person_Id))) as Array_Number) Person_Ids
                from Hac_Sync_Persons Dt
               group by Dt.Company_Id)
    loop
      v_Company := Gmap();
      v_Company.Put('company_id', r.Company_Id);
      v_Company.Put('person_ids', r.Person_Ids);
    
      v_Companies.Push(v_Company.Val);
    end loop;
  
    -- because final services put their data in array for some reason
    Result.Push(v_Companies);
  
    o_Output := Fazo.Read_Clob(Result.Val.To_Clob());
  end;

  ----------------------------------------------------------------------------------------------------
  Procedure Person_Sync_Response_Procedure(i_Input Array_Varchar2) is
  begin
    delete Hac_Sync_Persons;
  end;
