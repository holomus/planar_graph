create or replace package Ui_Vhr440 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Question_Group_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr440;
/
create or replace package body Ui_Vhr440 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Question_Group_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        ...
                        and q.question_group_id = :question_group_id',
    ...
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'question_group_id');
  ...
  begin
    ...
    result := z_Hln_Question_Groups.To_Map(r_Data,
                                           z.Question_Group_Id,
                                           z.Name,
                                           z.Code,
                                           z.Order_No,
    ...
    Result.Put('is_required', Md_Util.Decode(r_Data.Is_Required, 'Y', Ui.t_Yes, 'N', Ui.t_No));
