create or replace package Ui_Vhr38 is
  ----------------------------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------------
  Function Check_Data(p Arraylist) return Arraylist;
  ----------------------------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------------
  Procedure Run_One(p Hashmap);
  ----------------------------------------------------------------------------------------------------
  Procedure Run_Multiple(p Hashmap);
end Ui_Vhr38;
/
create or replace package body Ui_Vhr38 is
  ----------------------------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------------
  Function Check_Data(p Arraylist) return Arraylist is
    ...
    v_Hiring          Hashmap;
    v_Indicators      Hashmap;
    v_Oper_Kinds      Hashmap;
    v_Oper_Types      Arraylist;
    v_Ret_Oper_Types  Arraylist;
    v_Oper_Type       Hashmap;
    v_Indicator_Ids   Array_Number;
    v_Indicator_Names Array_Varchar2;
    v_Kinds           Array_Varchar2 := Array_Varchar2(Mpr_Pref.c_Ok_Accrual,
                                                       Mpr_Pref.c_Ok_Deduction);
    result            Arraylist := Arraylist();
  begin
    for i in 1 .. p.Count
    loop
      v_Hiring := Treat(p.r_Hashmap(i) as Hashmap);
    
      -- employee
      if not z_Mhr_Employees.Exist(i_Company_Id  => v_Company_Id,
                                   i_Filial_Id   => v_Filial_Id,
                                   i_Employee_Id => v_Hiring.o_Number('employee_id')) then
        v_Hiring.Put('employee_id', to_number(null));
        v_Hiring.Put('employee_name', to_char(null));
      end if;
    
      -- division
      if not z_Mhr_Divisions.Exist(i_Company_Id  => v_Company_Id,
                                   i_Filial_Id   => v_Filial_Id,
                                   i_Division_Id => v_Hiring.o_Number('division_id')) then
        v_Hiring.Put('division_id', to_number(null));
        v_Hiring.Put('division_name', to_char(null));
      end if;
    
      -- job
      if not z_Mhr_Jobs.Exist(i_Company_Id => v_Company_Id,
                              i_Filial_Id  => v_Filial_Id,
                              i_Job_Id     => v_Hiring.o_Number('job_id')) then
        v_Hiring.Put('job_id', to_number(null));
        v_Hiring.Put('job_name', to_char(null));
      end if;
    
      -- robot
      if not z_Mrf_Robots.Exist(i_Company_Id => v_Company_Id,
                                i_Filial_Id  => v_Filial_Id,
                                i_Robot_Id   => v_Hiring.o_Number('robot_id')) then
        v_Hiring.Put('robot_id', to_number(null));
        v_Hiring.Put('robot_name', to_char(null));
      end if;
    
      -- rank
      if not z_Mhr_Ranks.Exist(i_Company_Id => v_Company_Id,
                               i_Filial_Id  => v_Filial_Id,
                               i_Rank_Id    => v_Hiring.o_Number('rank_id')) then
        v_Hiring.Put('rank_id', to_number(null));
        v_Hiring.Put('rank_name', to_char(null));
      end if;
    
      -- wage scale
      if not z_Hrm_Wage_Scales.Exist(i_Company_Id    => v_Company_Id,
                                     i_Filial_Id     => v_Filial_Id,
                                     i_Wage_Scale_Id => v_Hiring.o_Number('wage_scale_id')) then
        v_Hiring.Put('wage_scale_id', to_number(null));
        v_Hiring.Put('wage_scale_name', to_char(null));
      end if;
    
      -- fte
      v_Fte_Id := v_Hiring.o_Number('fte_id');
    
      if not z_Href_Ftes.Exist(i_Company_Id => v_Company_Id, i_Fte_Id => v_Fte_Id) and
         v_Fte_Id <> Href_Pref.c_Custom_Fte_Id then
        r_Fte := z_Href_Ftes.Load(i_Company_Id => v_Company_Id,
                                  i_Fte_Id     => Href_Util.Fte_Id(i_Company_Id => v_Company_Id,
                                                                   i_Pcode      => Href_Pref.c_Pcode_Fte_Full_Time));
      
        v_Hiring.Put('fte_id', r_Fte.Fte_Id);
        v_Hiring.Put('fte_name', r_Fte.Name);
        v_Hiring.Put('fte_value', r_Fte.Fte_Value);
      end if;
    
      -- schedule
      if not z_Htt_Schedules.Exist(i_Company_Id  => v_Company_Id,
                                   i_Filial_Id   => v_Filial_Id,
                                   i_Schedule_Id => v_Hiring.o_Number('schedule_id')) then
        v_Hiring.Put('schedule_id', to_number(null));
        v_Hiring.Put('schedule_name', to_char(null));
      end if;
    
      -- employment source
      if not z_Href_Employment_Sources.Exist(i_Company_Id => v_Company_Id,
                                             i_Source_Id  => v_Hiring.o_Number('source_id')) then
        v_Hiring.Put('source_id', to_number(null));
        v_Hiring.Put('source_name', to_char(null));
      end if;
    
      -- fixed term base
      if not
          z_Href_Fixed_Term_Bases.Exist(i_Company_Id         => v_Company_Id,
                                        i_Fixed_Term_Base_Id => v_Hiring.o_Number('fixed_term_base_id')) then
        v_Hiring.Put('fixed_term_base_id', to_number(null));
        v_Hiring.Put('fixed_term_base_name', to_char(null));
      end if;
    
      -- oper type
      v_Ret_Oper_Types := Arraylist;
      v_Indicators     := v_Hiring.r_Hashmap('indicators');
      v_Oper_Kinds     := v_Hiring.r_Hashmap('oper_types');
    
      for Kind in 1 .. v_Kinds.Count
      loop
        v_Oper_Types := v_Oper_Kinds.r_Arraylist(v_Kinds(Kind));
      
        for j in 1 .. v_Oper_Types.Count
        loop
          v_Oper_Type    := Treat(v_Oper_Types.r_Hashmap(j) as Hashmap);
          v_Oper_Type_Id := v_Oper_Type.o_Number('oper_type_id');
        
          if z_Mpr_Oper_Types.Exist(i_Company_Id => v_Company_Id, i_Oper_Type_Id => v_Oper_Type_Id) then
            select q.Indicator_Id, q.Name
              bulk collect
              into v_Indicator_Ids, v_Indicator_Names
            ...
            v_Oper_Type.Put('indicator_ids', v_Indicator_Ids);
          
            for k in 1 .. v_Indicator_Ids.Count
            loop
              if v_Indicators.o_Hashmap(v_Indicator_Ids(k)) is null then
                v_Indicators.Put(v_Indicator_Ids(k),
                                 Fazo.Zip_Map('indicator_id',
                                              v_Indicator_Ids(k),
                                              'name',
                                              v_Indicator_Names(k)));
              end if;
            ...
            v_Oper_Type.Put('operation_kind', v_Kinds(Kind));
            v_Ret_Oper_Types.Push(v_Oper_Type);
          end if;
        end loop;
      end loop;
    
      v_Hiring.Put('oper_types', v_Ret_Oper_Types);
      v_Hiring.Put('indicators', v_Indicators);
    
      Result.Push(v_Hiring);
    end loop;
  ...
  begin
    select Array_Varchar2(Hir.Page_Id,
                           ...
                           Hir.Fixed_Term,
                           Hir.Expiry_Date,
                           Hir.Fixed_Term_Base_Id,
                           Hir.Fixed_Term_Base_Name,
                           Hir.Concluding_Term,
  ...
  begin
    ...
    Result.Put('indicator_wage_id', v_Indicator_Id);
    Result.Put('indicator_wage_name',
               z_Href_Indicators.Load(i_Company_Id => Ui.Company_Id, i_Indicator_Id => v_Indicator_Id).Name);
  ...
  begin
    ...
    loop
      ...
      Hpd_Util.Robot_New(o_Robot           => p_Robot,
                         ...
                         i_Division_Id     => v_Hiring.r_Number('division_id'),
                         ...
                         i_Employment_Type => v_Hiring.r_Varchar2('employment_type'),
      ...
      Hpd_Util.Contract_New(o_Contract             => p_Contract,
                            ...
                            i_Contract_Date        => v_Hiring.o_Date('contract_date'),
                            i_Fixed_Term           => v_Hiring.o_Varchar2('fixed_term'),
                            i_Expiry_Date          => v_Hiring.o_Date('expiry_date'),
                            i_Fixed_Term_Base_Id   => v_Hiring.o_Number('fixed_term_base_id'),
                            i_Concluding_Term      => v_Hiring.o_Varchar2('concluding_term'),
                            i_Hiring_Conditions    => v_Hiring.o_Varchar2('hiring_conditions'),
                            i_Other_Conditions     => v_Hiring.o_Varchar2('other_conditions'),
                            i_Workplace_Equipment  => v_Hiring.o_Varchar2('workplace_equipment'),
                            i_Representative_Basis => v_Hiring.o_Varchar2('representative_basis'));
      ...
      v_Oper_Types := v_Hiring.r_Arraylist('oper_types');
      p_Oper_Type  := Href_Pref.Oper_Type_Nt();
      ...
      Attach_Employee(v_Hiring.r_Number('employee_id'));
    
      Hpd_Util.Journal_Add_Hiring(p_Journal              => p_Journal,
                                  i_Page_Id              => v_Page_Id,
                                  i_Employee_Id          => v_Hiring.r_Number('employee_id'),
                                  i_Staff_Number         => v_Hiring.o_Varchar2('staff_number'),
                                  i_Hiring_Date          => v_Hiring.r_Date('hiring_date'),
    ...
    Hpd_Api.Hiring_Journal_Save(p_Journal, i_Delay_Repairing => i_Repost);
  ...
  ----------------------------------------------------------------------------------------------------
  Procedure Run_One(p Hashmap) is
  begin
    Ui.Grant_Check('run_one');
    Ui_Vhr106.Run(p);
  end;

  ----------------------------------------------------------------------------------------------------
  Procedure Run_Multiple(p Hashmap) is
  begin
    Ui.Grant_Check('run_multiple');
    Ui_Vhr108.Run(p);
  end;
