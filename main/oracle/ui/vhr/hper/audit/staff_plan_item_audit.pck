create or replace package Ui_Vhr204 is
  ---------------------------------------------------------------------------------------------------
  Function Query(p Hashmap) return Fazo_Query;
end Ui_Vhr204;
/
create or replace package body Ui_Vhr204 is
  ----------------------------------------------------------------------------------------------------
  ...
  begin
    q := Fazo_Query('select *
                       from x_hper_staff_plan_items q
    ...
    q.Number_Field('t_audit_id',
                   ...
                   'plan_type_id',
                   'plan_value');
    q.Number_Field('plan_amount', 'fact_value', 'fact_percent', 'fact_amount');
    ...
    v_Matrix := Hper_Util.Plan_Types;
  
    q.Option_Field('item_plan_type_name', 'plan_type', v_Matrix(1), v_Matrix(2));
  ...
  begin
    update x_Hper_Staff_Plan_Items
