create or replace package Ui_Vhr158 is
  ----------------------------------------------------------------------------------------------------
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr158;
/
create or replace package body Ui_Vhr158 is
  ----------------------------------------------------------------------------------------------------
  ...
  begin
    ...
    Uit_Href.Assert_Access_To_Staff(i_Staff_Id => r_Staff.Staff_Id, i_Filial_Id => v_Filial_Id);
    ...
    Result.Put('state_name', Md_Util.Decode(r_Staff.State, 'A', Ui.t_Active, 'P', Ui.t_Passive));
    Result.Put('status', v_Status);
    Result.Put('status_name', Href_Util.t_Staff_Status(v_Status));
    Result.Put('staff_kind_name', Href_Util.t_Staff_Kind(r_Staff.Staff_Kind));
    ...
    Result.Put('fte_name',
               z_Href_Ftes.Take(i_Company_Id => r_Staff.Company_Id, i_Fte_Id => r_Staff.Fte_Id).Name);
    Result.Put('created_by_name',
               z_Md_Users.Load(i_Company_Id => r_Staff.Company_Id, --
               i_User_Id => r_Staff.Created_By).Name);
    Result.Put('modified_by_name',
               z_Md_Users.Load(i_Company_Id => r_Staff.Company_Id, --
               i_User_Id => r_Staff.Modified_By).Name);
