create or replace package Ui_Vhr387 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Award_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr387;
/
create or replace package body Ui_Vhr387 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Award_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.award_id = :award_id',
                    Fazo.Zip_Map('company_id', Ui.Company_Id, 'award_id', p.r_Number('award_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'award_id');
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    result       Hashmap;
  begin
    r_Data := z_Href_Awards.Load(i_Company_Id => v_Company_Id, i_Award_Id => p.r_Number('award_id'));
  
    result := z_Href_Awards.To_Map(r_Data, z.Award_Id, z.Name, z.State, z.Created_On, z.Modified_On);
