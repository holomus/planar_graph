create or replace package Ui_Vhr375 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Doc_Type_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr375;
/
create or replace package body Ui_Vhr375 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Doc_Type_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.doc_type_id = :doc_type_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'doc_type_id',
                                 p.r_Number('doc_type_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'doc_type_id');
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    result       Hashmap;
  begin
    r_Data := z_Href_Document_Types.Load(i_Company_Id  => v_Company_Id,
                                         i_Doc_Type_Id => p.r_Number('doc_type_id'));
  
    result := z_Href_Document_Types.To_Map(r_Data,
                                           z.Doc_Type_Id,
                                           z.Name,
                                           z.Is_Required,
                                           z.State,
