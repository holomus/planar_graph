create or replace package Ui_Vhr358 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Edu_Stage_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr358;
/
create or replace package body Ui_Vhr358 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Edu_Stage_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.edu_stage_id = :edu_stage_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'edu_stage_id',
                                 p.r_Number('edu_stage_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'edu_stage_id');
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    result       Hashmap;
  begin
    r_Data := z_Href_Edu_Stages.Load(i_Company_Id   => v_Company_Id,
                                     i_Edu_Stage_Id => p.r_Number('edu_stage_id'));
  
    result := z_Href_Edu_Stages.To_Map(r_Data,
                                       z.Edu_Stage_Id,
                                       z.Name,
                                       z.State,
                                       z.Code,
