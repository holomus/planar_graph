create or replace package Ui_Vhr406 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Institution_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr406;
/
create or replace package body Ui_Vhr406 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Institution_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.institution_id = :institution_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'institution_id',
                                 p.r_Number('institution_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'institution_id');
  ...
  begin
    ...
    result := z_Href_Institutions.To_Map(r_Data,
                                         z.Institution_Id,
                                         z.Name,
                                         z.State,
                                         z.Code,
