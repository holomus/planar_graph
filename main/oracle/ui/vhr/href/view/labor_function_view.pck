create or replace package Ui_Vhr389 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Labor_Function_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr389;
/
create or replace package body Ui_Vhr389 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Labor_Function_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.labor_function_id = :labor_function_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'labor_function_id',
                                 p.r_Number('labor_function_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'labor_function_id');
    q.Varchar2_Field('t_event', 't_source_project_code', 'name', 'description', 'code');
  ...
  begin
    ...
    result := z_Href_Labor_Functions.To_Map(r_Data,
                                            z.Labor_Function_Id,
                                            z.Name,
                                            z.Description,
                                            z.Code,
