create or replace package Ui_Vhr372 is
  ----------------------------------------------------------------------------------------------------
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr372;
/
create or replace package body Ui_Vhr372 is
  ----------------------------------------------------------------------------------------------------
  Function t
  (
    i_Message varchar2,
  ...
  begin
    return b.Translate('UI-VHR372:' || i_Message, i_P1, i_P2, i_P3, i_P4, i_P5);
  end;
  ...
  begin
    ...
    loop
      ...
      v_Diff_Columns := z_x_Href_Langs.Difference(r, r_Last);
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    v_Lang_Id    number := p.r_Number('lang_id');
    v_Context_Id number := p.r_Number('context_id');
    result       Hashmap := Hashmap();
  begin
    Result.Put('name',
               z_Href_Langs.Take(i_Company_Id => v_Company_Id, i_Lang_Id => v_Lang_Id).Name);
    Result.Put('lang',
               Fazo.Zip_Matrix(Langs(i_Company_Id => v_Company_Id,
                                     i_Lang_Id    => v_Lang_Id,
