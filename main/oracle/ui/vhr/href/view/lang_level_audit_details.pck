create or replace package Ui_Vhr374 is
  ----------------------------------------------------------------------------------------------------
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr374;
/
create or replace package body Ui_Vhr374 is
  ----------------------------------------------------------------------------------------------------
  Function t
  (
    i_Message varchar2,
  ...
  begin
    return b.Translate('UI-VHR374:' || i_Message, i_P1, i_P2, i_P3, i_P4, i_P5);
  end;

  ----------------------------------------------------------------------------------------------------
  Function Lang_Levels
  (
    i_Company_Id    number,
    i_Lang_Level_Id number,
    i_Context_Id    number
  ) return Matrix_Varchar2 is
    r_Last         x_Href_Lang_Levels%rowtype;
  ...
  begin
    for r in (select q.*
                from x_Href_Lang_Levels q
               where q.t_Company_Id = i_Company_Id
                 and q.Lang_Level_Id = i_Lang_Level_Id
    ...
    loop
      if r.t_Event = 'U' then
        begin
          select *
            into r_Last
            from (select *
                    from x_Href_Lang_Levels t
                   where t.t_Company_Id = r.t_Company_Id
                     and t.Lang_Level_Id = r.Lang_Level_Id
      ...
      v_Diff_Columns := z_x_Href_Lang_Levels.Difference(r, r_Last);
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    v_Lang_Level_Id number := p.r_Number('lang_level_id');
    result          Hashmap := Hashmap();
  begin
    Result.Put('name',
               z_Href_Lang_Levels.Take(i_Company_Id => v_Company_Id, i_Lang_Level_Id => v_Lang_Level_Id).Name);
    Result.Put('lang_level',
               Fazo.Zip_Matrix(Lang_Levels(i_Company_Id    => v_Company_Id,
                                           i_Lang_Level_Id => v_Lang_Level_Id,
