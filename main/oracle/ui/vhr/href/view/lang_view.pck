create or replace package Ui_Vhr371 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Lang_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr371;
/
create or replace package body Ui_Vhr371 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Lang_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.lang_id = :lang_id',
                    Fazo.Zip_Map('company_id', Ui.Company_Id, 'lang_id', p.r_Number('lang_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'lang_id');
  ...
  begin
    ...
    result := z_Href_Langs.To_Map(r_Data,
                                  z.Lang_Id,
                                  z.Name,
                                  z.State,
                                  z.Code,
