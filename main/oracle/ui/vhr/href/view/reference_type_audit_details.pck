create or replace package Ui_Vhr378 is
  ----------------------------------------------------------------------------------------------------
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr378;
/
create or replace package body Ui_Vhr378 is
  ----------------------------------------------------------------------------------------------------
  Function t
  (
    i_Message varchar2,
  ...
  begin
    return b.Translate('UI-VHR378:' || i_Message, i_P1, i_P2, i_P3, i_P4, i_P5);
  end;
  ...
  (
    ...
    r_Last         x_Href_Reference_Types%rowtype;
  ...
  begin
    for r in (select q.*
                from x_Href_Reference_Types q
               where q.t_Company_Id = i_Company_Id
                 and q.Reference_Type_Id = i_Reference_Type_Id
    ...
    loop
      if r.t_Event = 'U' then
        begin
          select *
            into r_Last
            from (select *
                    from x_Href_Reference_Types t
                   where t.t_Company_Id = r.t_Company_Id
                     and t.Reference_Type_Id = r.Reference_Type_Id
      ...
      v_Diff_Columns := z_x_Href_Reference_Types.Difference(r, r_Last);
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    v_Reference_Type_Id number := p.r_Number('reference_type_id');
    result              Hashmap := Hashmap();
  begin
    Result.Put('name',
               z_Href_Reference_Types.Take(i_Company_Id => v_Company_Id, i_Reference_Type_Id => v_Reference_Type_Id).Name);
    Result.Put('reference_type',
               Fazo.Zip_Matrix(Reference_Types(i_Company_Id        => v_Company_Id,
                                               i_Reference_Type_Id => v_Reference_Type_Id,
