create or replace package Ui_Vhr377 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Reference_Type_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr377;
/
create or replace package body Ui_Vhr377 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Reference_Type_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.reference_type_id = :reference_type_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'reference_type_id',
                                 p.r_Number('reference_type_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'reference_type_id');
  ...
  begin
    ...
    result := z_Href_Reference_Types.To_Map(r_Data,
                                            z.Reference_Type_Id,
                                            z.Name,
                                            z.State,
                                            z.Code,
