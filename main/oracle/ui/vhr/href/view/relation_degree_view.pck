create or replace package Ui_Vhr381 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Relation_Degree_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr381;
/
create or replace package body Ui_Vhr381 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Relation_Degree_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.relation_degree_id = :relation_degree_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'relation_degree_id',
                                 p.r_Number('relation_degree_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'relation_degree_id');
  ...
  begin
    ...
    result := z_Href_Relation_Degrees.To_Map(r_Data,
  ...
  begin
    update x_Href_Relation_Degrees
