create or replace package Ui_Vhr361 is
  ----------------------------------------------------------------------------------------------------
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr361;
/
create or replace package body Ui_Vhr361 is
  ----------------------------------------------------------------------------------------------------
  Function t
  (
    i_Message varchar2,
  ...
  begin
    return b.Translate('UI-VHR361:' || i_Message, i_P1, i_P2, i_P3, i_P4, i_P5);
  end;

  ----------------------------------------------------------------------------------------------------
  Function Science_Branchs
  (
    i_Company_Id        number,
    i_Science_Branch_Id number,
    i_Context_Id        number
  ) return Matrix_Varchar2 is
    r_Last         x_Href_Science_Branches%rowtype;
  ...
  begin
    for r in (select q.*
               ...
               where q.t_Company_Id = i_Company_Id
                 and q.Science_Branch_Id = i_Science_Branch_Id
    ...
    loop
      if r.t_Event = 'U' then
        begin
          select *
            into r_Last
            from (select *
                    from x_Href_Science_Branches t
                   where t.t_Company_Id = r.t_Company_Id
                     and t.Science_Branch_Id = r.Science_Branch_Id
      ...
      v_Diff_Columns := z_x_Href_Science_Branches.Difference(r, r_Last);
  ...
  Function Model(p Hashmap) return Hashmap is
    ...
    v_Science_Branch_Id number := p.r_Number('science_branch_id');
  begin
    Result.Put('name',
               z_Href_Science_Branches.Take(i_Company_Id => v_Company_Id, i_Science_Branch_Id => v_Science_Branch_Id).Name);
    Result.Put('science_branch',
               Fazo.Zip_Matrix(Science_Branchs(i_Company_Id        => Ui.Company_Id,
                                               i_Science_Branch_Id => v_Science_Branch_Id,
