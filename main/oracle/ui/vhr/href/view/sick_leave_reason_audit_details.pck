create or replace package Ui_Vhr393 is
  ----------------------------------------------------------------------------------------------------
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr393;
/
create or replace package body Ui_Vhr393 is
  ----------------------------------------------------------------------------------------------------
  Function t
  (
    i_Message varchar2,
  ...
  begin
    return b.Translate('UI-VHR393:' || i_Message, i_P1, i_P2, i_P3, i_P4, i_P5);
  end;

  ----------------------------------------------------------------------------------------------------
  Function Sick_Leave_Reasons
  (
    i_Company_Id number,
    i_Filial_Id  number,
    i_Reason_Id  number,
    ...
    r_Last         x_Href_Sick_Leave_Reasons%rowtype;
  ...
  begin
    ...
    loop
      if r.t_Event = 'U' then
        begin
          select *
            into r_Last
            from (select *
                    from x_Href_Sick_Leave_Reasons t
      ...
      v_Diff_Columns := z_x_Href_Sick_Leave_Reasons.Difference(r, r_Last);
  ...
  begin
    Result.Put('name',
               z_Href_Sick_Leave_Reasons.Take(i_Company_Id => v_Company_Id, i_Filial_Id => v_Filial_Id, i_Reason_Id => v_Reason_Id).Name);
    Result.Put('reason',
               Fazo.Zip_Matrix(Sick_Leave_Reasons(i_Company_Id => v_Company_Id,
