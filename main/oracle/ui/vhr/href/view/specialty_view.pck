create or replace package Ui_Vhr367 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Specialty_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr367;
/
create or replace package body Ui_Vhr367 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Specialty_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                    ...
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'specialty_id',
                                 p.r_Number('specialty_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'specialty_id');
    q.Varchar2_Field('t_event', 't_source_project_code', 'name', 'kind', 'state', 'code');
    ...
    v_Matrix := Href_Util.Specialty_Kinds;
  ...
  begin
    ...
    result := z_Href_Specialties.To_Map(r_Data,
                                        z.Specialty_Id,
                                        z.Name,
                                        z.State,
                                        z.Code,
    ...
    Result.Put('kind_name', Href_Util.t_Specialty_Kind(r_Data.Kind));
    Result.Put('parent_name',
               z_Href_Specialties.Take(i_Company_Id => r_Data.Company_Id, i_Specialty_Id => r_Data.Parent_Id).Name);
