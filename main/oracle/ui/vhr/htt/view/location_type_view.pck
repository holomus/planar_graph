create or replace package Ui_Vhr411 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Location_Type_Audit(p Hashmap) return Fazo_Query;
  ---------------------------------------------------------------------------------------------------- 
  Function Model(p Hashmap) return Hashmap;
end Ui_Vhr411;
/
create or replace package body Ui_Vhr411 is
  ----------------------------------------------------------------------------------------------------
  Function Query_Location_Type_Audit(p Hashmap) return Fazo_Query is
  ...
  begin
    q := Fazo_Query('select q.*
                      ...
                      where q.t_company_id = :company_id
                        and q.location_type_id = :location_type_id',
                    Fazo.Zip_Map('company_id',
                                 Ui.Company_Id,
                                 'location_type_id',
                                 p.r_Number('location_type_id')));
  
    q.Number_Field('t_audit_id', 't_user_id', 't_context_id', 'location_type_id');
    q.Varchar2_Field('t_event', 't_source_project_code', 'name', 'color', 'state', 'code');
  ...
  Function Model(p Hashmap) return Hashmap is
    r_Data Htt_Location_Types%rowtype;
    result Hashmap;
  begin
    r_Data := z_Htt_Location_Types.Load(i_Company_Id       => Ui.Company_Id,
                                        i_Location_Type_Id => p.r_Number('location_type_id'));
  
    result := z_Htt_Location_Types.To_Map(r_Data,
                                          z.Location_Type_Id,
                                          z.Name,
                                          z.Color,
                                          z.State,
