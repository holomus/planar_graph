create or replace package Hr5_Migr_Core is
  ----------------------------------------------------------------------------------------------------
  Procedure Run_Migr;
end Hr5_Migr_Core;
/
create or replace package body Hr5_Migr_Core is
  ----------------------------------------------------------------------------------------------------
  Procedure Run_Migr is
  ...
  begin
    ...
    Hr5_Migr_Pref.g_Begin_Date := Trunc(sysdate, 'mon');
    Hr5_Migr_Pref.g_End_Date   := Last_Day(Hr5_Migr_Pref.g_Begin_Date);
    ...
    Hr5_Migr_Robot.Migr_Business_Trip_Post(v_Company_Id);
    ...
    Hr5_Migr_Api.Job_Log('ok');
  
  exception
    when others then
      rollback;
    
      Hr5_Migr_Api.Job_Log(Dbms_Utility.Format_Error_Stack || ' ' ||
